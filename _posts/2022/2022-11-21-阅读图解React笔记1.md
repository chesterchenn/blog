...
2. react-dom
   ...
   react 渲染器之一，将 react-reconciler 的运行结果输出到 web 上。基本上只用到 ReactDOM.render(element, container) 入口函数。

3. react-reconciler

   react 核心包，协调 react, react-dom, scheduler 之间的调度。将输入信号最终转换成输出信号传递给渲染器。

   - 接受输入，将 fiber 树生成逻辑封装到一个回调函数中。
   - 将此毁掉函数送入 scheduler 进行调度。
   - scheduler 会控制回调函数的执行时机，回调函数执行完成后得到全新的 fiber 树。
   - 再调用渲染器 react-dom 将 fiber 树形结构最终反映到界面上。

4. scheduler

   调度机制，控制由 react-reconciler 送入的回调函数执行时机，在 concurrent 模式下可以实现任务分片。在 concurrent 模式下可实现中断渲染。
